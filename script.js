...
const handleRegister = () => {
    ...
    if (!usernameInput || !mobInput || !passwordInput || !confirmPasswordInput) {
        alert("One or more input fields are missing!");
        return;
    }

    let username = usernameInput.value;
    let mob = mobInput.value;
...
const handleLogin = () => {
    ...
    if (!usernameInput || !passwordInput) {
        alert("Login input fields are missing!");
        return;
    }
    ...
    let user_found = reg_user_list.some((v) => {
        return user_name === v.name && user_password === v.password;
    });
...
for (let i = 0; i < show_products.length; i++) {
    ...
    id_tag.innerText = show_products[i].id;
    title_tag.innerText = show_products[i].title;
    price_tag.innerText = show_products[i].price;
    image_tag.src = show_products[i].image;
...
function handleAll() {
    products.map((product) => {
        ...
        image_tag.style.maxWidth = "100%";
        image_tag.style.height = "auto";
        image_tag.style.marginBottom = "10px"
...
const loadHomeProducts = () => {
  let main = document.getElementById("PRODUCTS");
  main.innerHTML = ""; // Clear if reloaded

  // You can change slice(0, 5) to select any 5
  products.slice(0, 21).forEach(product => {
    let container = document.createElement("div");
    container.className = "product-card";

    let id_tag = document.createElement("p");
    id_tag.innerText = `ID: ${product.id}`;

    let title_tag = document.createElement("h6");
    title_tag.innerText = product.title;

    let price_tag = document.createElement("p");
    price_tag.innerText = `Price: $${product.price}`;
